...
function openProductModal(productId) {
    ...
    const sizesArr  = (product.sizes && product.sizes.length) ? product.sizes : ['52','54','56','58','60'];
    select.innerHTML = '<option value="">اختاري القياس...</option>' +
    ...
    document.querySelectorAll('#sliderInner .slider-slide').forEach(img => {
        img.onclick = () => {
            ...
            const zImg = document.getElementById('zoomedImage');
            if (z && zImg) { zImg.src = currentImages[currentIndex]; z.style.display='flex'; requestAnimationFrame(()=>z.style.opacity='1'); }
        };
    });
...
function setupSwipe(el, onLeft, onRight) {
    ...
    el.addEventListener('touchend', e => {
        ...
        if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 44) {
            // RTL: swipe left = go to next (forward in Arabic reading)
            if (dx < 0) onLeft();
            else        onRight();
        }
    }, { passive: true });
    ...
    <div id="productModal" class="modal-overlay" onclick="closeProductModal(event)">
        <div class="modal-content product-modal-inner" onclick="event.stopPropagation()">
            ...
            <div class="modal-product-info">
                ...
                <select id="sizeSelect" class="size-select-dropdown"></select>
