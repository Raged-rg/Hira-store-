            ...
            <div class="product-image-container" onclick="openProductModal('${product.id}')">
                <div class="product-img-wrapper">
                    <img
                        ...
                        alt="${product.name}"
                        class="product-img"
                        style="object-position:${product.imagePositionX||50}% ${product.imagePositionY||50}%;transform:scale(${product.imageScale||1});"
...
function setupSearch() {
    ...
    input.addEventListener('input', e => {
        ...
        renderProducts(q ? hiraProducts.filter(p => p.name.toLowerCase().includes(q)) : hiraProducts);
    });
}
...
function updateDetailSlider(direction) {
    ...
    slides.forEach((s, i) => {
        ...
        s.classList.toggle('prev-slide', i !== idx);
    });
...
function renderCartItems() {
    ...
    hira_cart.forEach((item, index) => {
        ...
        const posX  = p ? (p.imagePositionX||50) : 50;
        const posY  = p ? (p.imagePositionY||50) : 50;
...
/* ============================================================
   PRODUCT MODAL (Quick-View / Lightbox)
   ============================================================ */
...
function updateSlider() {
    ...
    slides.forEach((s, i) => {
        ...
        s.classList.toggle('prev-slide', i !== currentIndex);
    });
...
function renderThumbnails() {
    ...
    currentImages.forEach((img, i) => {
        ...
        t.src = img;
        t.loading = 'lazy';
        t.className = `thumbnail-item ${i===0?'active':''}`;
        t.alt = `صورة ${i+1}`;
        t.onclick = () => { currentIndex = i; updateSlider(); resetAutoSlider(); };
...
function openProductModal(productId) {
    ...
    // In-place Zoom on image click
    document.querySelectorAll('#sliderInner .slider-slide').forEach(img => {
        img.onclick = () => {
            if (isZoomed) {
                resetZoom();
                resetAutoSlider(); // resume slider
            } else {
                zoomScale = 1.8;
                isZoomed = true;
                panX = 0;
                ...
                clearInterval(autoSliderInterval); // pause auto slider while zoomed
            }
        };
    });
...
function addToCartFromModal() {
    ...
    if (!size) { showCustomAlert('يرجى اختيار القياس', 'اختار القياس', 'حسنًا'); return; }

    const p   = window.currentProduct;
    const img = (p.images && p.images.length) ? p.images[0] : p.image;
    pushToCart(p.id, p.name, p.price, size, img);
    triggerCardPop(p.id);
    closeProductModal();
}
...
function setupSwipe(el, onLeft, onRight) {
    ...
    el.addEventListener('touchstart', e => {
        if (e.touches.length === 2) {
            ...
            if (isZoomed) {
                const img = document.querySelector('#sliderInner .slider-slide.active');
    ...
    el.addEventListener('touchmove', e => {
        if (e.touches.length === 2) {
            ...
            zoomScale = Math.max(1, Math.min(initialScale * (dist / initialDist), 2.5));
            ...
            // Limit pan bounds so image doesn't fly off screen
            const maxPanX = (el.clientWidth / 2) * (zoomScale - 1) / zoomScale;
            const maxPanY = (el.clientHeight / 2) * (zoomScale - 1) / zoomScale;
...
function injectModals() {
    ...
    <div id="productModal" class="modal-overlay" onclick="closeProductModal(event)">
        <div class="modal-content product-modal-inner" onclick="event.stopPropagation()">
            <button id="closeModal" class="modal-close" onclick="closeProductModal()" aria-label="إغلاق">
                <i class="fa fa-times"></i>
            </button>

            <div class="slider-container" id="modalSlider">
                <div class="slider-inner" id="sliderInner"></div>
                <div id="modalNavArea"></div>
            </div>
