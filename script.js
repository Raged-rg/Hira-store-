...
function renderProducts(productsToRender) {
    ...
    limitedProducts.forEach(product => {
        ...
        card.setAttribute('aria-label', `عرض منتج ${product.name}`);
            ...
            <div class="product-image-container">
                <div class="product-img-wrapper">
                    <img
                        ...
                        onerror="console.log('Image failed to load:', this.src); this.src='https://via.placeholder.com/400x600?text=Error'; this.onerror=null;"
                    >
                </div>
...
function renderProductDetail(productId) {
    ...
    const slidesHTML = images.map((img, i) =>
        `<img src="${img}" class="slider-slide ${i===0?'active':''}" alt="${product.name}" loading="${i===0?'eager':'lazy'}" crossorigin="anonymous" onclick="openZoomDetail()" onerror="console.log('Image failed to load:', this.src); this.src='https://via.placeholder.com/600x800?text=Error'; this.onerror=null;">`
    ).join('');

    const thumbsHTML = images.length > 1
        ? images.map((img, i) =>
            `<img src="${img}" class="thumbnail-item ${i===0?'active':''}" loading="lazy" crossorigin="anonymous" onclick="setDetailSlide(${i})" alt="صورة ${i+1}" onerror="console.log('Image failed to load:', this.src); this.src='https://via.placeholder.com/100x150?text=Error'; this.onerror=null;">`
          ).join('')
        : '';
        ...
        container.innerHTML += `
            <div class="cart-item" id="fav-row-${p.id}">
                <div class="cart-item-thumb" onclick="closeFavModal();setTimeout(()=>openProductModal('${p.id}'),120);">
                    <img src="${p.image}" loading="lazy" crossorigin="anonymous" class="cart-item-img" style="object-position:${p.imagePositionX||50}% ${p.imagePositionY||50}%;" onerror="console.log('Image failed to load:', this.src); this.src='https://via.placeholder.com/100x150?text=Error'; this.onerror=null;">
                </div>
        ...
        container.innerHTML += `
            <div class="cart-item">
                <div class="cart-item-thumb">
                    <img src="${item.image}" loading="lazy" crossorigin="anonymous" class="cart-item-img" style="object-position:${posX}% ${posY}%;" onerror="console.log('Image failed to load:', this.src); this.src='https://via.placeholder.com/100x150?text=Error'; this.onerror=null;">
                </div>
                <div class="cart-item-details">
...
function initSlider(images) {
    ...
    if (inner) {
        inner.innerHTML = currentImages.map((img, i) =>
            `<img src="${img}" class="slider-slide ${i===0?'active':''}" loading="${i===0?'eager':'lazy'}" crossorigin="anonymous" alt="صورة المنتج" onerror="console.log('Image failed to load:', this.src); this.src='https://via.placeholder.com/600x800?text=Error'; this.onerror=null;">`
        ).join('');
